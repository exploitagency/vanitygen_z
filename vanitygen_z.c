...
//							Don't overwrite the 2nd prefix byte.  
...
{
	fprintf(stderr,
"Vanitygen_z %s (" OPENSSL_VERSION_TEXT ")\n"
"Usage: %s [-vqnrik1] [-t <threads>] [-f <filename>|-] [<pattern>...]\n"
"Generates a Zcash receiving address matching <pattern>, and outputs the address\n"
"and associated private key.  The private key may be stored in a safe location\n"
"or imported into a Zcash client to spend any balance received on the address.\n"
...
//	int scriptaddrtype = 5;
	...
	while ((opt = getopt(argc, argv, "vqnrik1eE:P:NTXF:t:h?f:o:s:")) != -1) {
		switch (opt) {
		case 'v':
			verbose = 2;
			break;
